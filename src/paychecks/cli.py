...
def validate(
    ...
    frequency: Annotated[
        str,
        typer.Option("--frequency", help="Pay frequency: weekly, biweekly, semimonthly, monthly"),
    ],
    salary_change: Annotated[
        list[str] | None, typer.Option("--salary-change", help="Mid-year change: YYYY-MM-DD:AMOUNT")
    ] = None,
    tolerance: Annotated[
        float, typer.Option("--tolerance", help="Per-field tolerance in USD")
    ] = 0.02,
    output: Annotated[
        Path | None, typer.Option("--output", help="Save report to .txt or .csv file")
    ] = None,
) -> None:
...
def reconcile(
    ...
    salary_change: Annotated[list[str] | None, typer.Option("--salary-change")] = None,
    ...
    pdf_files = sorted(p for p in paychecks_dir.glob("*.pdf") if p.resolve() != w2_pdf.resolve())
    if not pdf_files:
    ...
    with Progress(SpinnerColumn(), text_col) as progress:
        task = progress.add_task(
            f"Extracting {len(pdf_files)} paycheck PDFs…", total=len(pdf_files)
        )
