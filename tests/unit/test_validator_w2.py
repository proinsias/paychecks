...
from paychecks.models import (
    Deduction, ExtractionMethod, Paycheck, PayFrequency, SalarySchedule, ValidationStatus
)
...
def make_w2(annual: float = 120_000, tax_year: int = 2025, **overrides) -> W2:
    ...
    return W2(
        ...
        box2_federal_tax_withheld=overrides.get("box2", (salary * Decimal("0.22")).quantize(Decimal("0.01"))),
        box3_social_security_wages=overrides.get("box3", salary),
        box4_social_security_tax=overrides.get("box4", (salary * Decimal("0.062")).quantize(Decimal("0.01"))),
        box5_medicare_wages=overrides.get("box5", salary),
        box6_medicare_tax=overrides.get("box6", (salary * Decimal("0.0145")).quantize(Decimal("0.01"))),
        box16_state_wages=overrides.get("box16", salary),
        box17_state_tax=overrides.get("box17", (salary * Decimal("0.06")).quantize(Decimal("0.01"))),
...
class TestW2Reconciliation:
    def _make_year_paychecks(self, annual: float = 120_000, periods: int = 26) -> list:
        ...
        start = date(2025, 1, 1)
        return [
            make_paycheck(start + timedelta(days=i * 14), gross, annual)
    ...
    def test_mismatch_detected(self):
        ...
        box2_field = next(f for f in report.fields if "federal" in f.field_name.lower() or "box 2" in f.field_name.lower())
